...
if($selected_region = $purchase_log['shipping_country'] != null) {
	$selected_country = $purchase_log['shipping_country'];
	$selected_region = $purchase_log['shipping_region'];
} else {
	$selected_country = $purchase_log['billing_country'];
	$selected_region = $purchase_log['billing_region'];
}
$region_list = $wpdb->get_row("SELECT `".WPSC_TABLE_REGION_TAX."`.* FROM `".WPSC_TABLE_REGION_TAX."`, `".WPSC_TABLE_CURRENCY_LIST."`  WHERE `".WPSC_TABLE_CURRENCY_LIST."`.`isocode` IN('".$selected_country."') AND `".WPSC_TABLE_CURRENCY_LIST."`.`id` = `".WPSC_TABLE_REGION_TAX."`.`country_id` AND `".WPSC_TABLE_REGION_TAX."`.`id` IN ('".$selected_region."') LIMIT 1",ARRAY_A) ;
if($region_list != null) {
	$selected_region_code = $region_list['code'];
} else {
	$selected_region_code = 0;
}
...
$authorize_data['x_State'] = urlencode($selected_region_code);
$authorize_data['x_Country'] = urlencode($selected_country);
